...
export function InstallLayout({
  ...
  const toggleMute = (e: React.MouseEvent) => {
    e.stopPropagation(); // Previne acionar o handleInteraction do fundo
    if (isMuted) {
      setIsMuted(false);
      audioStartedRef.current = true;
      startAmbient();
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-50">
        <button
          onClick={toggleMute}
          className="p-2 rounded-full border border-[var(--br-muted-cyan)]/30 text-[var(--br-muted-cyan)] hover:text-[var(--br-neon-cyan)] hover:border-[var(--br-neon-cyan)] hover:bg-[var(--br-neon-cyan)]/10 transition-all shadow-[0_0_10px_rgba(0,0,0,0.5)] bg-[var(--br-void-black)]/50 backdrop-blur"
          title={isMuted ? "Tocar música ambiente" : "Pausar música ambiente"}
        >
