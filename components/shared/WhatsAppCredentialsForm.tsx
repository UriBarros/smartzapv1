...
export interface WhatsAppCredentials {
  // Legacy Meta Cloud API fields (optional/deprecated)
...
export function WhatsAppCredentialsForm({
  ...
  // Verifica se os campos obrigatórios estão preenchidos para uma das APIs
  const hasMetaCreds = Boolean(
    values.phoneNumberId?.trim() &&
    values.businessAccountId?.trim() &&
    values.accessToken?.trim()
  )
  const hasEvolutionCreds = Boolean(
    values.evolutionApiUrl?.trim() &&
    values.evolutionApiKey?.trim() &&
    values.evolutionInstanceName?.trim()
  )
  const canTest = hasMetaCreds || hasEvolutionCreds
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    try {
      const payload = hasEvolutionCreds
        ? {
          evolutionApiUrl: values.evolutionApiUrl?.trim(),
          evolutionApiKey: values.evolutionApiKey?.trim(),
          evolutionInstanceName: values.evolutionInstanceName?.trim(),
        }
        : {
          phoneNumberId: values.phoneNumberId?.trim(),
          businessAccountId: values.businessAccountId?.trim(),
          accessToken: values.accessToken ? sanitizeAccessToken(values.accessToken) : '',
        }
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className={cn(
        ...
        <div className="space-y-4 pt-4 border-t border-[var(--ds-border-subtle)]">
          <h3 className="font-semibold text-sm text-[var(--ds-text-primary)]">Conexão Evolution API (Primária)</h3>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => handleFieldChange('evolutionApiUrl', e.target.value)}
          ...
          <div className="space-y-2">
            <Label htmlFor="evolutionApiKey" className="flex items-center gap-2">
              Global API Key ou Instance Token
            </Label>
            <Input
              id="evolutionApiKey"
              type="password"
              ...
              onChange={(e) => handleFieldChange('evolutionApiKey', e.target.value)}
              disabled={isTesting || isSaving}
              className="font-mono"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ex: smartzap-instance"
              value={values.evolutionInstanceName || ''}
              onChange={(e) => handleFieldChange('evolutionInstanceName', e.target.value)}
              disabled={isTesting || isSaving}
              className="font-mono"
            />
          </div>
        ...
        <div className="space-y-4 pt-6 border-t border-[var(--ds-border-subtle)]">
          <h3 className="font-semibold text-sm text-[var(--ds-text-secondary)]">Conexão Meta API (Secundária/Fallback)</h3>
          {/* Phone Number ID */}
          <div className="space-y-2">
