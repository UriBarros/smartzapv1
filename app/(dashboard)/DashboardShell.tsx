        ...
        <PageLayoutProvider>
            <div className="min-h-screen bg-[var(--ds-bg-base)] text-[var(--ds-text-primary)] flex font-sans selection:bg-primary-500/30">
                ...
                <div className={cn(
                    "flex-1 flex flex-col min-w-0 h-screen overflow-hidden transition-[padding] duration-200",
                    isSidebarExpanded && "lg:pl-56",
                    "bg-[var(--ds-bg-elevated)]" // Subtle background contrast
                )}>
                    {/* Header */}
                    <header className="h-16 flex items-center justify-between px-6 lg:px-12 shrink-0 bg-[var(--ds-bg-base)] border-b border-[var(--ds-border-subtle)] sticky top-0 z-10">
                        <div className="flex items-center">
                            <button
                                className="lg:hidden p-2 text-[var(--ds-text-secondary)] mr-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary-500 focus-visible:outline-offset-2 rounded-md hover:bg-[var(--ds-bg-hover)] transition-colors"
                                onClick={() => {
                                    updateSidebarExpanded(true)
                                    setIsMobileMenuOpen(true)
                                }}
...
function PageContentShell({ children }: { children: React.ReactNode }) {
    ...
    const mainPaddingClass = layout.padded ? 'p-6 lg:p-12' : ''
    const wrapperWidthClass = getPageWidthClass(layout.width)
    const wrapperHeightClass = layout.height === 'full' ? 'h-full' : ''

    return (
        <main className={`flex-1 ${mainOverflowClass} ${mainPaddingClass} bg-[var(--ds-bg-elevated)]`.trim()}>
            <div className={cn(
                wrapperWidthClass,
                wrapperHeightClass,
                layout.padded && "bg-[var(--ds-bg-base)] rounded-2xl border border-[var(--ds-border-subtle)] shadow-sm p-6 lg:p-8 min-h-[calc(100vh-8rem)]" // Inner card
            )}>
